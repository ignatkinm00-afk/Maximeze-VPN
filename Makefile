...
BINDIR=maximeze-vpn-core$(SEP)bin
ANDROID_OUT=android$(SEP)app$(SEP)libs
IOS_OUT=ios$(SEP)Frameworks
DESKTOP_OUT=maximeze-vpn-core$(SEP)bin
GEO_ASSETS_DIR=assets$(SEP)core

CORE_PRODUCT_NAME=maximeze-vpn-core
CORE_NAME=maximeze-vpn-lib
LIB_NAME=maximeze-vpn-core

ifeq ($(CHANNEL),prod)
	CORE_URL=https://github.com/maximeze-vpn/maximeze-vpn-core/releases/download/v$(core.version)
else
	CORE_URL=https://github.com/maximeze-vpn/maximeze-vpn-core/releases/download/draft
endif
...
generate_kotlin_protos: 
	...
	# protoc \
	# 	--proto_path=maximeze-vpn-core/ \
	# 	--java_out=./android/app/src/main/java/ \
	# 	--grpc-java_out=./android/app/src/main/java/ \
	# 	$(shell find maximeze-vpn-core/v2 maximeze-vpn-core/extension -name "*.proto")
	rsync -av --delete \
		--include='*/' \
		--include='*.proto' \
		--exclude='*' \
		maximeze-vpn-core/v2 maximeze-vpn-core/extension ./android/app/src/main/protos/
	# # Find .proto files and update package declarations
	# find "./android/app/src/main/java/com/maximeze-vpn/maximeze-vpn/protos" -type f -name "*.java" | while read -r proto_file; do \
	#     if grep -q "^package " "$$proto_file"; then \
	#         $(SED) 's/^package \([\w\.]*\)/package com.maximeze-vpn.maximeze-vpn.protos.\1/g' "$$proto_file"; \
	#     fi \
	# done

generate_go_protoc:
	make -C maximeze-vpn-core -f Makefile protos
	echo "SED: $(SED)"
generate_dart_protoc:
	mkdir -p lib/maximeze-vpncore/generated
	protoc --dart_out=grpc:lib/maximeze-vpncore/generated --proto_path=maximeze-vpn-core/  $(shell find maximeze-vpn-core/v2 maximeze-vpn-core/extension -name "*.proto") 	google/protobuf/timestamp.proto ; \
...
windows-zip-release:
	...
	mkdir -p Maximeze VPN; \
	unzip -q "$$ZIP_FILE" -d Maximeze VPN/; \
	rm "$$ZIP_FILE"; \
	tar -a -cf "$$FILE_NAME.zip" Maximeze VPN; \
	rm -rf Maximeze VPN; \
...
linux-appimage-release:
	...
	$(BLUE)Adding StartupWMClass to maximeze-vpn.desktop$(DONE); \
	sed -i '/^\[Desktop Entry\]/a StartupWMClass=app.maximeze-vpn.com' "squashfs-root/maximeze-vpn.desktop"; \
	...
	ARCH=x86_64 appimagetool --no-appstream squashfs-root Maximeze VPN.AppImage > /dev/null; \
	$(BLUE)Cleaning up squashfs$(DONE); \
	rm -rf squashfs-root; \
	$(YELLOW)Creating Portable Package$(DONE); \
	PKG_DIR_NAME="maximeze-vpn-linux-appimage"; \
	$(BLUE)Creating dir: $$PKG_DIR_NAME$(DONE); \
	mkdir -p "$$PKG_DIR_NAME"; \
	$(BLUE)Moving Maximeze VPN.AppImage$(DONE); \
	cp -p "Maximeze VPN.AppImage" "$$PKG_DIR_NAME/Maximeze VPN.AppImage"; \
	$(BLUE)Creating Portable Home directory$(DONE); \
	mkdir -p "$$PKG_DIR_NAME/Maximeze VPN.AppImage.home"; \
...
DOCKER_IMAGE_NAME := maximeze-vpn-linux-builder
DOCKER_FLUTTER_VOL := maximeze-vpn-flutter-sdk-cache
DOCKER_PUB_VOL := maximeze-vpn-pub-cache
...
ios-libs: #not tested
	mkdir -p $(IOS_OUT)
	rm -rf $(IOS_OUT)/Maximeze VPNCore.xcframework
...
build-headers:
	make -C maximeze-vpn-core -f Makefile headers && mv $(BINDIR)/$(CORE_NAME)-headers.h $(BINDIR)/maximeze-vpn-core.h

build-android-libs:
	make -C maximeze-vpn-core -f Makefile android 
	mv $(BINDIR)/$(LIB_NAME).aar $(ANDROID_OUT)/

build-windows-libs:
	make -C maximeze-vpn-core -f Makefile windows-amd64

build-linux-libs:
	make -C maximeze-vpn-core -f Makefile linux-amd64 

build-macos-libs:
	make -C maximeze-vpn-core -f Makefile macos

build-ios-libs: 
	rm -rf $(IOS_OUT)/Maximeze VPNCore.xcframework 
	make -C maximeze-vpn-core -f Makefile ios  
	mv $(BINDIR)/Maximeze VPNCore.xcframework $(IOS_OUT)/Maximeze VPNCore.xcframework
